...
- **Coding sessions**: un repo activo por chat en `./workspace/<slug>`. Solo `https://github.com|gitlab.com`. `git push` prohibido en `assertSafeGitArgs`. Token privado: `GITHUB_TOKEN`/`GITLAB_TOKEN` vía `http.extraHeader`, nunca en la URL ni logs. n8n timeout de `/chat` es 45s → jobs async + `sendMessage` del mismo bot (`TELEGRAM_CHAT_ID` only).
