...
- **Roadmap 07–10 cerrado, no reabrir**: las fases vivas están en `roadmap/07_*.md` … `roadmap/10_*.md`. No re-ejecutes `roadmap/archive/` (bootstrap 1–6). DoD 07 cumplido: estado de Murray es `murray.db` WAL, no JSON. Siguen pendientes: dedup de triage por static data de n8n → `/triage/filter` + `/triage/mark-seen` (fase 08); URL `api.deepseek.com` en Murray/OpenHands → contenedor `litellm` (fase 09). Hasta esos DoD, static data y DeepSeek directo siguen siendo la verdad de runtime.
