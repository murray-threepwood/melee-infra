...
- **Roadmap 07–10 cerrado, no reabrir**: las fases vivas están en `roadmap/07_*.md` … `roadmap/10_*.md`. No re-ejecutes `roadmap/archive/` (bootstrap 1–6). DoD 07–10 cumplidos: `murray.db` WAL; triage `/triage/filter` + `/triage/mark-seen`; gateway `litellm`; TTL 30 min de `oh-agent-server-*`. El endurecimiento del socket Docker sigue en `roadmap/90_BACKLOG_HARDENING.md`.
...
- **`/jobs` no es `docker logs`**: lista la tabla `jobs` de `murray.db` (clone/code/git/delete/oh_poll). El «log» son notify de Telegram + `status=done|failed|stuck|paused`, cap 40 guardadas / 20 al mostrar. No sustituye `/logs n8n`. Slash e intercept, sin LLM. Reloj y duración salen de `createdAt` (America/Montevideo); `updatedAt` miente en `oh_poll` porque el worker lo toca cada ~2s. Id Murray = 16 hex; conversation/start-task OpenHands = 32 hex: `find` acepta los tres. Un UUID pegado o «en qué quedó task …» diagnostica ese job. «qué pasa» / `/triage` es el informe del obrero, no el dump del último job. JSON `jobs.json` / `memory.json` / `session.json` solo existen como origen de migración one-shot (`*.migrated`).
- **OpenHands `sandbox PAUSED` no es done**: `execution_status` null + sandbox PAUSED no pasa `isAgentDone`. Árbol sucio → job `paused` + aviso de commit. Limpio o repo ausente → stuck HITL `sandbox_paused`, nunca «misión lista». `finished` + git vacío → `empty_finish`. Avisar al CEO al pausar, trancar y terminar, no solo si pregunta.
...
- **Heal de obrero no es `docker rm` libre**: `/triage` emite HITL `heal_openhands`. Solo borra contenedores `oh-agent-server-*` (nombre, no ancestor `runtime`) y reinicia `openhands`. `propose_ops` no tiene ese action. El panel `el_corazon` opción 3 usa el mismo filtro de nombre.
