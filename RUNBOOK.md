...
- Verificación: con misión en vuelo, Murray manda Telegram al pausar, trancar o terminar. `/triage` o «qué pasó» encola `inspect`. `/jobs` o UUID 32 hex en slash sigue siendo detalle crudo (`sandbox=PAUSED`). Árbol sucio → status `paused` (no HITL stuck). Limpio → teclado Reintentar/Parar. `seguí con L01` re-arma HITL. `node --test tests/test_jobs.mjs tests/test_coding_session.mjs tests/test_murray_agent_http.mjs tests/test_inspect.mjs`.
...
- Verificación: Telegram `/triage` o «qué pasó» ACK job inspect (dump + modelo + auto-ops allowlist, P12). Tareas humanas en `operator-inbox/`. `/jobs <id>` sigue crudo. `node --test tests/test_inspect.mjs tests/test_triage.mjs tests/test_jobs.mjs tests/test_coding_session.mjs tests/test_murray_agent_http.mjs`.
